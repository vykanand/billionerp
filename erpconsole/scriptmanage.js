...
function ddl(iss){
  $.get("../deletemodule.php?dest="+iss, function(data, status){
   console.log(data);
   // var notie = window.notie
   // notie.alert({ type: 'success', text: iss+' App Deleted', stay: false })
   window.top.postMessage('success^App Deleted', '*')
   localStorage.removeItem('liinfo')
